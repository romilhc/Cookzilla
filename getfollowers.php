		...
		if (!isset($_SESSION['user'])){
		  echo "Enter Username Please";
			...
			/*if($us===$_SESSION['user'])
		      ...
		      {*/
		        $sql =
		        "SELECT user.uname,fname,lname,street,city,country,state,zip,propic from user join follows on user.uname=follows.uname where funame = '".$us."'";
		      //}
			...
			if ($result->num_rows === 0) { 
				...
				<strong>Oh snap! There are no followers.</strong>
				</div>';
				...
				while ($row = $result->fetch_assoc()) {
					...
					echo "<tr><td><div class='col-sm-2'><div class='thumbnail'><img class='img-responsive user-photo' src='data:image;base64,".$row['propic']."'>				  </div></div></td><td><h5>" . $row['fname'].' '.$row['lname']. "</h5></td>";
                   ...
                   if($row['uname']===$_SESSION['user'])
				  {
				  	echo'</td></tr>';
						...
						echo"<td><a href='profile.php?uname=".$row['uname']."' class='btn btn-primary btn-sm'>View Profile</a></td></tr>";
					}
