  ...
  <body>
		...
		if (!isset($_SESSION['user'])){
		  echo "Enter Username Please";
			...
			if ($result->num_rows === 0) { 
				...
				<strong>Oh great! You belongs to all the group.</strong> 
				</div>';
				...
				while ($row = $result->fetch_assoc()) {
				  ...
				  echo "<tr><td>" . $row["gname"]."</td><td>" . $row["desc"]. "</td><td>" . $row["nou"]."</td><td><a href='addToGroup.php?gid=".$row['gid']."' class='btn btn-primary btn-sm'>Join!</a></td><td><a href='viewgmem.php?gid=".$row['gid']."' class='btn btn-primary btn-sm'>View Members!</a></td></tr>";
	...
	<?php
		if(isset($_POST['grpid']))
		{
			$grpid=$_POST['grpid'];
			echo $grpid;
			header('location: getrestgroup.php');
		}
